  ...
  return (
    <>
      ...
      <section className="container  max-w-7xl mx-auto px-[120] relative ">
        <Image src={info} alt="" className="absolute right-1/2 top-[60%] translate-x-1/2 -translate-y-[30%] -z-10" width={800} height={800} />
      ...
      <section className=" bg-white py-[100px] px-[120px] min-w-screen">
        ...
        <div className="container mx-auto max-w-7xl grid grid-cols-1 sm:grd-cols-2 lg:grid-cols-3 gap-6 p-4">
          {blogData.slice(0, 3).map((item: ItemProps, ind: number) => (
            <Link href={`/blog/${item._id}`} key={item._id} className='border border-[#DFDFDF] p-6 rounded-lg w-full max-w-[384px] mx-auto block hover:shadow-md transition-shadow relative h-[463px]'>
              <Image
                src={item.image}
                alt={item.title}
                width={384}
              ...
              <div className="p-4">
                ...
                <div className="text-[#151515] flex items-center gap-2 mt-4 absolute bottom-6">
                  <Image
